...
{
	struct mtcp_epoll_event ev;
	uint8_t raw_buf[MAX_HEADER_LEN];	// buffer before decrypt & MAC
	...
	/* Get payload to raw_buf */
	rd = mtcp_read(ctx->mctx, sockid, (char *)raw_buf, MAX_HEADER_LEN);
	if (rd <= 0) {
		return rd;
	}

#if VERBOSE
	fprintf(stdout, "Got a packet!!\n");
	for (i = 0; i < rd; i++)
		fprintf(stdout, "%02X%c", raw_buf[i],
				((i + 1) % 16) ? ' ' : '\n');
	fprintf(stdout, "\n");
	fflush(stdout);
#endif /* VERBOSE */
...
#if FAST_RESET
	AbortConnection(ctx, sockid, sv);
#endif /* FAST_RESET */
	/* Decrypt & MAC from raw_buf to buf */
...
AcceptConnection(struct thread_context *ctx, int listener)
{
	...
	info = (ssl_info_t *)calloc(1, sizeof(ssl_info_t));
	c = mtcp_accept_tls(mctx, listener, NULL, NULL, info);
	...
	if (c >= 0) {
		...
		if (sv->ssl_info) {
			fprintf(stdout, "ssl_info check(key): \n");
			fprintf(stdout, "client_write_MAC_secret:\n");
			for (z = 0; z < sv->ssl_info->mac_key_size; z++)
				fprintf(stdout, "%02X%c", sv->ssl_info->client_write_MAC_secret[z],
						((z + 1) % 16) ? ' ' : '\n');
			fprintf(stdout, "\n");
		}
		else {
			fprintf(stdout, "NIC Overloaded!\n");
		}
#endif
		ev.events = MTCP_EPOLLIN;
		ev.data.sockid = c;
