...
/*----------------------------------------------------------------------------*/
static inline tcp_stream *
HandlePassiveOpenByMeta(mtcp_manager_t mtcp, uint32_t cur_ts, const struct iphdr *iph, 
		const struct tcphdr *tcph, uint32_t next_recv_seq, uint16_t window)
{
	tcp_stream *cur_stream = NULL;
	...
	cur_stream->rcvvar->irs = next_recv_seq - 1;
	cur_stream->rcv_nxt = cur_stream->rcvvar->irs;
	cur_stream->sndvar->cwnd = cur_stream->sndvar->mss * 10;
...
/*----------------------------------------------------------------------------*/
static inline tcp_stream *
CreateNewFlowHTEntryByMeta(mtcp_manager_t mtcp, uint32_t cur_ts,
				...
				int payloadlen, uint16_t window)
{
	...
	/* Filter as SYN packet */	
	ret = FilterSYNPacket(mtcp, iph->daddr, tcph->dest);

	if (!ret) {
		TRACE_ERROR("Refusing Meta.\n");
#ifdef DBGMSG
		DumpIPPacket(mtcp, iph, ip_len);
#endif
		SendTCPPacketStandalone(mtcp, 
				iph->daddr, tcph->dest, iph->saddr, tcph->source, 
				0, next_recv_seq, 0, TCP_FLAG_RST | TCP_FLAG_ACK, 
				NULL, 0, cur_ts, 0);
	...
	/* now accept the connection */
	cur_stream = HandlePassiveOpenByMeta(mtcp, cur_ts, iph,
						tcph, next_recv_seq, window);
	if (!cur_stream) {
		TRACE_DBG("Not available space in flow pool.\n");
		SendTCPPacketStandalone(mtcp, 
				iph->daddr, tcph->dest, iph->saddr, tcph->source, 
...
static inline int
decrypt_aes(uint8_t *ciphertext, int ciphertext_len, uint8_t *key,
...
int
ProcessTCPMetaPacket(mtcp_manager_t mtcp, uint32_t cur_ts, const int ifidx,
			const struct iphdr *iph, int ip_len)
{
	...
	uint32_t next_recv_seq = ntohl(tcph->seq);
	uint32_t next_recv_ack = ntohl(tcph->ack_seq);
	...
	if (payloadlen == 0) {
		fprintf(stdout, "\n[CPU %d][META IN]\n"
				...
				next_recv_seq, next_recv_ack, ntohs(tcph->window));
	}
	else {
		fprintf(stdout, "\n[CPU %d][META IN]\n"
				"client addr: %x\n"
	...
	if (!(cur_stream = StreamHTSearch(mtcp->tcp_flow_table, &s_stream))) {
		...
		cur_stream = CreateNewFlowHTEntryByMeta(mtcp, cur_ts, iph, ip_len, tcph, 
				next_recv_seq, next_recv_ack, payloadlen, window);
		if (!cur_stream)
			return TRUE;
	}
	else {
		/* There should not be this connection */
		/* int cur_sockid = cur_stream->socket->id; */
		/* fprintf(stderr, "Metadata Connection Exists!!, tcp state: %u, closed: %u\n" */
		/* 		"sockid: %d, client_ip: %x, client_port: %d \n", */
		/* 		cur_stream->state, cur_stream->closed, */
		/* 		cur_sockid, htonl(cur_stream->daddr), htons(cur_stream->dport)); */

		/* regard this as rst packet */
		ProcessRST(mtcp, cur_stream, tcph->ack_seq);
		return TRUE;
	}
	...
	sndvar->peer_wnd = window;

	cur_stream->last_active_ts = cur_ts;
	UpdateTimeoutList(mtcp, cur_stream);

	/* Refer Handle_TCP_ST_CUSTOM_LISTEN */
	assert(cur_stream->state == TCP_ST_LISTEN);
	sndvar->iss = next_recv_ack - 1;
	sndvar->snd_una = next_recv_ack;
	sndvar->eff_mss = 1460;
	cur_stream->snd_nxt = next_recv_ack;
#if VERBOSE
	fprintf(stdout, "Set snd_next to %d\n", next_recv_ack);
#endif /* VERBOSE */
	/* irs and rcv_nxt is already set */

	sndvar->wscale_peer = 7;
	sndvar->wscale_mine = 7;

	/* Set ssl_onload TRUE and get information from meta */
	if (payloadlen > 0) {
		cur_stream->ssl_onload = TRUE;
		cur_stream->ssl_info = (ssl_info_t *)calloc(1, sizeof(ssl_info_t));

#if ENCRYPT_META
		ret = decrypt_aes(payload, payloadlen, nic_key, nic_iv, (uint8_t *)cur_stream->ssl_info);
#else /* ENCRYPT_META */
		memcpy(cur_stream->ssl_info, payload, sizeof(ssl_info_t));
#endif /* !ENCRYPT_META */
		cur_stream->ssl_info->active = TRUE;
#if VERBOSE
		int z;
#if ENCRYPT_META
		fprintf(stdout, "\nOriginal Payload(%d): \n", payloadlen);
		{
			for (z = 0; z < payloadlen; z++)
				fprintf(stdout, "%02X%c", payload[z],
						((z + 1) % 16) ? ' ' : '\n');
			fprintf(stdout, "\n");
		}
		...
		{
			for (z = 0; z < payloadlen; z++)
				fprintf(stdout, "%02X%c", ((uint8_t *)cur_stream->ssl_info)[z],
						((z + 1) % 16) ? ' ' : '\n');
			fprintf(stdout, "\n");
		}
#endif /* ENCRYPT_META */
		...
		fprintf(stdout, "session id: %u\n"
				"version: %u.%u\n",
				cur_stream->ssl_info->session_id,
				cur_stream->ssl_info->version.major, cur_stream->ssl_info->version.minor);
	
		fprintf(stdout, "\nmac_key_size: %u\n", cur_stream->ssl_info->mac_key_size);
		fprintf(stdout, "client_write_MAC_secret:\n");
		for (z = 0; z < cur_stream->ssl_info->mac_key_size; z++)
			fprintf(stdout, "%02X%c", cur_stream->ssl_info->client_write_MAC_secret[z],
					((z + 1) % 16) ? ' ' : '\n');
		fprintf(stdout, "\n");
		fprintf(stdout, "server_write_MAC_secret:\n");
		for (z = 0; z < cur_stream->ssl_info->mac_key_size; z++)
			fprintf(stdout, "%02X%c", cur_stream->ssl_info->server_write_MAC_secret[z],
					((z + 1) % 16) ? ' ' : '\n');
		fprintf(stdout, "\n");
	
		fprintf(stdout, "\nenc_key_size: %u\n", cur_stream->ssl_info->enc_key_size);
		fprintf(stdout, "client_write_key:\n");
		for (z = 0; z < cur_stream->ssl_info->enc_key_size; z++)
			fprintf(stdout, "%02X%c", cur_stream->ssl_info->client_write_key[z],
					((z + 1) % 16) ? ' ' : '\n');
		fprintf(stdout, "\n");
		fprintf(stdout, "server_write_key:\n");
		for (z = 0; z < cur_stream->ssl_info->enc_key_size; z++)
			fprintf(stdout, "%02X%c", cur_stream->ssl_info->server_write_key[z],
					((z + 1) % 16) ? ' ' : '\n');
		fprintf(stdout, "\n");

		fprintf(stdout, "\nenc_iv_size: %u\n", cur_stream->ssl_info->fixed_iv_length);
		fprintf(stdout, "client_write_iv:\n");
		for (z = 0; z < cur_stream->ssl_info->fixed_iv_length; z++)
			fprintf(stdout, "%02X%c", cur_stream->ssl_info->client_write_IV[z],
					((z + 1) % 16) ? ' ' : '\n');
		fprintf(stdout, "\n");
		fprintf(stdout, "server_write_iv:\n");
		for (z = 0; z < cur_stream->ssl_info->fixed_iv_length; z++)
			fprintf(stdout, "%02X%c", cur_stream->ssl_info->server_write_IV[z],
					((z + 1) % 16) ? ' ' : '\n');
		fprintf(stdout, "\n");
	...
	if (ret < 0) {
		...
		fprintf(stdout, "Connection %d: Epoll event raised.\n", cur_stream->id);
		fflush(stdout);
	}
#endif /* VERBOSE */
		 ...
		 uint32_t cur_ts, const int ifidx, const struct iphdr *iph, int ip_len)
{
...
#if VERBOSE
	fprintf(stdout, "\n[CPU %d][PKT IN] %x : %u -> %x : %u\n"
			"A: %u P: %u R: %u S: %u F: %u\n"
			"seq: %u, ack: %u, window: %u, payload_len: %d\n",
			mtcp->ctx->cpu,
			ntohl(iph->saddr), ntohs(tcph->source),
			ntohl(iph->daddr), ntohs(tcph->dest),
	...
	if (cur_stream->state > TCP_ST_SYN_RCVD) {
		...
		if (!ret) {
			/* Do not show this because ab sometimes gives inaccurate(?) timestamp (or we do not support) */
			/* fprintf(stdout, "Stream %d: Unexpected sequence: %u, expected: %u\n", */
			/* 		cur_stream->id, seq, cur_stream->rcv_nxt); */
#ifdef DBGMSG
			DumpIPPacket(mtcp, iph, ip_len);
#endif
