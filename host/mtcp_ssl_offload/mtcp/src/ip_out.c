		...
		uint16_t ip_id, uint32_t saddr, uint32_t daddr, uint16_t payloadlen)
{
	...
	iph = (struct iphdr *)EthernetOutput(mtcp, 
    		 ETH_P_IP, nif, haddr, payloadlen + IP_HEADER_LEN, NULL);
	if (!iph) {
		return NULL;
	}
...
/*----------------------------------------------------------------------------*/
uint8_t *
IPOutput(struct mtcp_manager *mtcp, tcp_stream *stream, uint16_t tcplen, uint8_t **opaque)
{
	struct iphdr *iph;
	int nif;
	...
	iph = (struct iphdr *)EthernetOutput(mtcp, ETH_P_IP, 
		   stream->sndvar->nif_out, haddr, tcplen + IP_HEADER_LEN, opaque);
	if (!iph) {
		return NULL;
	}
