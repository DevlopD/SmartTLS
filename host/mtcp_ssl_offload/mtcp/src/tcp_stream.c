...
#if SSL_OFF
	/* Initialize SSL offload related variables */
	stream->ssl_onload = FALSE;
	stream->ssl_info = NULL;
#endif /* SSL_OFF */
...
#ifdef USE_BLUEFIELD
	SendMetaDestroyPacket(mtcp, stream);
	TRACE_STREAM("[Destroy TCP Stream] sent MetaDestroy packet, %d\n");
#endif /* USE_BLUEFIELD */
