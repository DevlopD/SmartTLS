...
#if GSO_ENABLED
inline int
dpdk_mark_gso_pkt(struct mtcp_thread_context *ctxt, uint8_t *opaque)
{
	if(!opaque) {
		return -1;
	}

	struct rte_mbuf *m = (struct rte_mbuf*)opaque;
	...
	m->ol_flags |=
		PKT_TX_IPV4 | PKT_TX_IP_CKSUM | PKT_TX_TCP_CKSUM | PKT_TX_TCP_SEG;
...
#if GSO_ENABLED
uint8_t *
dpdk_get_gso_wptr(struct mtcp_thread_context *ctxt, int ifidx, uint16_t pktsize, uint8_t **opaque)
{
	struct dpdk_private_context *dpc;
#ifdef NETSTAT
	mtcp_manager_t mtcp;
...
#if GSO_ENABLED
	.get_gso_wptr      = dpdk_get_gso_wptr,
	.mark_gso_pkt      = dpdk_mark_gso_pkt,
#endif
