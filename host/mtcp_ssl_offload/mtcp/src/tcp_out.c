...
#if VERBOSE
	struct iphdr *iph = (struct iphdr *)((uint8_t *)tcph - 20);
	fprintf(stdout, "\n[CPU %d][PKT OUT (STANDALONE)] %x : %u -> %x : %u\n"
			"A: %u P: %u R: %u S: %u F: %u\n"
			"seq: %u, ack: %u, payload_len: %d\n",
			mtcp->ctx->cpu,
		...
		uint32_t cur_ts, uint8_t flags, uint8_t *payload, uint16_t payloadlen)
{
	...
	tcph = (struct tcphdr *)IPOutput(mtcp, cur_stream, 
			TCP_HEADER_LEN + optlen + payloadlen, &opaque);
#else
	tcph = (struct tcphdr *)IPOutput(mtcp, cur_stream, 
...
#if GSO_ENABLED
	mtcp->iom->mark_gso_pkt(mtcp->ctx, opaque);
#endif

#if VERBOSE
	struct iphdr *iph = (struct iphdr *)((uint8_t *)tcph - 20);
	fprintf(stdout, "\n[CPU %d][PKT OUT] %x : %u -> %x : %u\n"
			"A: %u P: %u R: %u S: %u F: %u\n"
			"seq: %u, ack: %u, payload_len: %d\n",
			mtcp->ctx->cpu,
			ntohl(iph->saddr), ntohs(tcph->source),
...
#if USE_BLUEFIELD
int
SendMetaDestroyPacket(struct mtcp_manager *mtcp, tcp_stream *cur_stream)
{
	struct tcphdr *tcph;
	struct iphdr *iph;
	uint8_t wscale = 0;
	...
	/* change type of service to mark indication of session destroy */
	iph = (struct iphdr *)((uint8_t *)tcph - 20);
	iph->tos = 0xff;			/* meta destroy marking */
