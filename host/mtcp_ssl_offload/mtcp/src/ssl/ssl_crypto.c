    ...
    const int out_len,    uint8_t *out)
{
    ...
    uint8_t *hash_result = buf;
    int real_seed_len = label_len + seed_len;
    uint8_t *real_seed = hash_result + out_len + 20;
    ...
    P_HASH(hash_func(), secret_len, secret,
           ...
           out_len, hash_result);
    /* P_HASH(EVhash_result(), secret_len, secret, */
    /*        real_seed_len, real_seed, */
    /*        out_len, hash_result); */

    memcpy(out, hash_result, out_len);
...
static int
get_key_bits(RSA* rsa)
{
    return RSA_size(rsa) * 8;
}

static void
priv_decrypt(RSA* rsa, unsigned char *out, int *out_len,
             const unsigned char *in, int in_len)
{
    int bytes_needed = get_key_bits(rsa) / 8;
    ...
    *out_len = RSA_private_decrypt(in_len, in, out, rsa,
	...
	rsa_cnt++;
	fprintf(stderr, "[priv_decrypt] RSA decryption takes %ld nano sec\n"
			"total encrypt count: %d\n",
...
static inline int
encrypt_aes_cbc(EVP_CIPHER_CTX *ctx, unsigned char *plaintext, int plaintext_len, unsigned char *key,
            unsigned char *iv, unsigned char *ciphertext)
{
    ...
    len = EVP_Cipher(ctx, ciphertext, plaintext, plaintext_len);
                ...
                unsigned char *tag, unsigned char *ciphertext)
{
	...
	if (elapsed_time > 1000)
		fprintf(stderr, "[encrypt aes gcm] EVP_EncryptUpdate (AAD) takes %ld nano sec\n",
		(tv_end.tv_nsec - tv_start.tv_nsec) + (tv_end.tv_sec - tv_start.tv_sec)*100000000);
	...
	elapsed_time = (tv_end.tv_nsec - tv_start.tv_nsec) + (tv_end.tv_sec - tv_start.tv_sec)*100000000;
	if (elapsed_time > 1000)
		fprintf(stderr, "[encrypt aes gcm] encrypting %d bytes takes %ld nano sec\n",
		plaintext_len,
		elapsed_time);
#endif
				...
				unsigned char *tag, unsigned char *plaintext)
{
	...
	elapsed_time = (tv_end.tv_nsec - tv_start.tv_nsec) + (tv_end.tv_sec - tv_start.tv_sec)*100000000;
	if (elapsed_time > 1000)
		fprintf(stderr, "[encrypt aes gcm] decrypting %d bytes takes %ld nano sec\n",
		ciphertext_len,
		elapsed_time);
#endif
...
#endif /* VERBOSE_KEY */
		int out_len = op->out_len;
		RSA* rsa = (RSA *)op->key;
		if (!rsa)
			return -1;
		priv_decrypt(rsa, op->out, &out_len, op->in, op->in_len);
		op->out_len = out_len;
		if (out_len == -1)
			return -1;
	}
	...
	if (op->opcode.s.function == TLS_AES_CBC) {
		if (op->opcode.s.op == ENCRYPT) {
			int ret = encrypt_aes_cbc(ctx,
								  ...
								  op->out);
			if (ret <= 0) {
				fprintf(stderr, "AES encryption cbc error\n");
				return -1;
			}
#if VERBOSE_AES
			...
			int ret = decrypt_aes_cbc(ctx,
								  ...
								  op->out);
			if (ret <= 0) {
				fprintf(stderr, "AES decryption cbc error\n");
				return -1;
			}
#if VERBOSE_AES
